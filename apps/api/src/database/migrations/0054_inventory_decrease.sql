CREATE FUNCTION inventory_api.apply_decrease(
...
BEGIN
  ...
  IF v_role NOT IN ('OWNER', 'ADMIN') OR v_role IS NULL THEN
    RAISE EXCEPTION 'inventory decrease forbidden' USING ERRCODE = '42501';
  END IF;
  ...
  INSERT INTO inventory_adjustments
    ...
    VALUES (p_id, p_organization_id, p_branch_id, p_item_id, p_actor_user_id,
      'DECREASE', p_quantity, p_reason, p_observation);
  INSERT INTO inventory_movements
    ...
    VALUES (gen_random_uuid(), p_organization_id, p_branch_id, p_item_id, p_actor_user_id,
      -p_quantity, 'ADJUSTMENT', p_id, p_id, 'DECREASE');
...
REVOKE ALL ON FUNCTION inventory_api.apply_decrease(uuid,uuid,uuid,uuid,uuid,numeric,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION inventory_api.apply_decrease(uuid,uuid,uuid,uuid,uuid,numeric,text,text) TO uco_app;
